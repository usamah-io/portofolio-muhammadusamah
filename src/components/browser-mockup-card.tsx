...
export default function BrowserMockupCard({
  ...
  return (
    <div
      ...
      } rounded-xl sm:rounded-2xl overflow-hidden transition-all duration-300 flex flex-col justify-between ${
        isPortrait ? "col-span-2 md:col-span-1" : "col-span-1"
      } shadow-sm hover:shadow-2xl hover:shadow-emerald-500/10 dark:hover:shadow-[#00FF87]/10 active:scale-[0.98] transform hover:-translate-y-1 sm:hover:-translate-y-1.5 ${className}`}
    >
      ...
      {/* Responsive Dual Layout Wrapper */}
      <div className={`flex-1 flex ${isPortrait ? "flex-row md:flex-col" : "flex-col"}`}>
        {/* 2. Media Container - Native HTML5 Video Player with Proxy Stream */}
        <div
          className={`relative overflow-hidden bg-zinc-950 dark:bg-black/95 group/media flex items-center justify-center transition-all duration-300 ${
            isPortrait
              ? "w-5/12 sm:w-1/2 md:w-full shrink-0 aspect-[3/4] sm:aspect-[9/16] md:aspect-auto"
              : "w-full"
          }`}
        >
          {mediaType === "image" ? (
            ...
            <div className="relative w-full h-full min-h-[160px] bg-zinc-950 flex flex-col items-center justify-center p-3 text-center space-y-2">
              {poster && (
                <img src={poster} alt={title} className="absolute inset-0 w-full h-full object-cover opacity-30" />
              )}
              <div className="relative z-10 p-2 rounded-full bg-zinc-900/90 border border-zinc-700 text-emerald-400 dark:text-[#00FF87]">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  <polygon points="5 3 19 12 5 21 5 3" />
                </svg>
              </div>
              <p className="relative z-10 text-[9px] sm:text-xs font-mono text-zinc-300 max-w-[160px]">
              ...
              <a
                ...
                className="relative z-10 inline-flex items-center gap-1 text-[9px] sm:text-xs font-bold text-zinc-950 bg-emerald-500 dark:bg-[#00FF87] px-2 py-1 rounded hover:bg-emerald-400 dark:hover:bg-[#00e67a] transition-colors"
              >
                Tonton di Drive ↗
              </a>
            </div>
            ...
            <video
              ...
              className="w-full h-auto object-cover bg-black rounded-t-xl"
            >
        ...
        <div
          className={`${
            isPortrait ? "w-7/12 sm:w-1/2 md:w-full" : "w-full"
          } p-3 sm:p-4 md:p-6 flex-1 flex flex-col justify-between space-y-2 sm:space-y-4 bg-white dark:bg-[#111111]`}
        >
          <div>
