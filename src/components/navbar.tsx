  ...
  return (
    <header className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-5xl px-4">
      ...
      <div className="flex items-center justify-between bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border border-zinc-200 dark:border-zinc-850/50 rounded-full px-6 py-3 shadow-2xl shadow-black/5 dark:shadow-black/40 transition-colors duration-300">
        ...
        <Link 
          href="/" 
          className="text-zinc-900 dark:text-white font-extrabold tracking-wider text-sm sm:text-base hover:opacity-80 transition-opacity"
        >
          DEV<span className="text-emerald-500">.</span>PORTFOLIO
        </Link>
        ...
        {/* Action Buttons, Switches & Mobile Trigger */}
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className="md:hidden p-1.5 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white rounded-full bg-zinc-100 dark:bg-zinc-800/50 hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors"
      ...
      {isOpen && (
        <div className="md:hidden mt-2 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-lg border border-zinc-250/60 dark:border-zinc-850/60 rounded-2xl p-4 flex flex-col gap-3 shadow-2xl animate-in fade-in slide-in-from-top-2 duration-255 transition-colors duration-300">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              onClick={() => setIsOpen(false)}
              className="text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white text-sm font-medium py-2 px-3 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors"
            >
              {link.label}
            </Link>
          ))}
          ...
          <Link
            href="/admin"
            onClick={() => setIsOpen(false)}
            className="text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white text-sm font-medium py-2 px-3 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors flex items-center gap-2 border-t border-zinc-100 dark:border-zinc-900 pt-3 mt-1"
          >
